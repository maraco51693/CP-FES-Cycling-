function xOut = anpatch( Action, hTarget, u )
%ANPATCH Generic patch creation and display function.
%	ANPATCH(ACTION,TARGET,U) is the creation and display 
%	function for the patch object in the simulation animation block.
%
...
case '#Draw',
    ...
    %    anpatch( #Draw, hObj, u ) redraw object hObj using data vector u    
    %---------------------------------------------------------------------------
...
case '#MouseClick',
    ...
    %---------------------------------------------------------------------------
    % Valid call combinations (ACTION-specific variable names):
    %    anpatch #MouseClick   (hTarget assumed to be gcbo)
    %---------------------------------------------------------------------------
...
case '#Modify',
    ...
    %
    % Valid call combinations (ACTION-specific variable names):
    %  anpatch( #Modify,  hObj       ) modify the obj (hFig = grandparent of hObj)
    %  anpatch( #Modify,  hObj, hFig ) modify the obj (hFig for free)
    %---------------------------------------------------------------------------
...
case '#ChangeLook',
    ...
    %
    % Valid call combinations (ACTION-specific variable names):
    %   anpatch( #UpdateIC, hObj, hUI ) update hObj based on uicontrol hUI
    %---------------------------------------------------------------------------
...
case '#UpdateIC',
    ...
    %                     OR []
    % Valid call combinations (ACTION-specific variable names):
    %   anpatch( #UpdateIC, hObj,          ) update hObj based on Position
    %   anpatch( #UpdateIC, hObj, sObjData ) update hObj based on sObjData
    %---------------------------------------------------------------------------
...
if isstruct( sObjectData ),
    ...
    % Make a tipped-L-shaped line
    XPos = XPos + [0 0.65 0.40 0.7 0.0]*Wide;
    YPos = YPos + [0 0.00 0.25 0.8 0.8]*High;
    ;
...
%-------------------------------------------------------------------------------
cTextStrings = { 'X Coordinate of vertices:'
...
%-------------------------------------------------------------------------------
FigTitle  = [ 'Patch Object Properties: ', ...
...
if isempty( s ),
    ...
    if DiffX | DiffY,
        s.XPos = [ OldXString ' + ' num2str( DiffX(1) ) ];
        s.YPos = [ OldYString ' + ' num2str( DiffY(1) ) ];
    end % if DiffX | DiffY
...
if findstr('u(', s.Color), DrawStr = [ DrawStr ',''FaceColor'',' s.Color]; end
