...
case '#SimCmd',
    ...
    if exist('Power') == 1
        ...
        prompt = {'Subject #?','Timing (PRE,POST,etc.)?','Type of test (CONST,INCR,WIN,etc)?', 'Loaded(enter LOAD or UNLOAD)?'};
        dlgTitle = 'File Name';
        numlines = 1;
        default = {'000','PREPOST','CONSTINCR','LOADUNLOAD'};
        ...
        if isempty(userinfo) == 0 
            ...
            Filename = strcat('FESCYCLING_',sub,'_',reltime,'_',testype,'_',resist,'_',datime)
        %     
        % %     prompt = {'Enter the path for the saved .mat file?                          '}
        % %     dlgTitle = 'Path for new .mat file';
        % %     numlines = 1;
        % %     default = {strcat('C:\Documents and Settings\research\My Documents\CP Cycling\Constant_Load_Test\')};
        % %     path_file = char(inputdlg(prompt,dlgTitle,numlines,default));
        % %     cd path_file
        % 
        %             save(Filename)
        end
...
case 'stopping',    
    ...
    prompt = {'Subject #?','When? (PRE,POST,etc.)','Type of test? (CONST,INCR,WIN,etc)', 'Loaded? (Enter loaded or unloaded)'};
%     prompt = {'Subject #?','When? (PRE,POST,etc.)','Type of test? (forcepedals)', 'Loaded? (Enter loaded or unloaded)','Trial #? (1,2,...)'};

    dlgTitle = 'File Name';
    numlines = 1;
%     default = {'000','PREPOST','INCR','loadedunloaded','1'};
    default = {'000','PREPOST','INCR','loadedunloaded'};
    userinfo = inputdlg(prompt,dlgTitle,numlines,default);
