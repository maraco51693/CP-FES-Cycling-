...
% ------------------------
% Specially added for the FES Cycling block set
% an update of the individual block directories
% onto the main matlab path
%disp(['Info (', mfilename, '): Updating the matlab path for the FES Cycling Block Set']);
FES_Cycling_Lib_open(0);
% ------------------------

% Name of the subsystem which will show up in the Simulink Blocksets
% and Toolboxes subsystem
blkStruct.Name = sprintf('FES_Cycling\nBlock Set');

% The function that will be called when the user double-clicks on
% this icon.
% Example:  blkStruct.OpenFcn = 'dsplib';
%
blkStruct.OpenFcn = 'FES_Cycling_Lib_open(1)';  %.m-File that opens the current version of the Lib.
...
Browser(1).Library = 'FES_Cycling_Lib';
Browser(1).Name    = 'FES Cycling Block Set';
